...
class MealPlanWeekViewModel {
    
    var mealPlan: MealPlanWeek? {
        didSet {
    ...
    func getMealPlanDay(calories: String) {
        ...
        self.isLoading = true
        self.contentAlpha = 0
        let url = Endpoints.mealPlanWeek(calories: calories).url
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now()+0.7) {
            ApiServices.shared.getData(url: url, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: nil) { [weak self] (result: MealPlanWeek?, err) in
